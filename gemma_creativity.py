#!/usr/bin/env python3
"""
ITI Creativity Analysis for Gemma-7b-it
...
SAVE_DIR = "features/creativity_gemma_7b"
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
BATCH_SIZE = 1

print("="*60)
print("GEMMA-7B CREATIVITY ITI ANALYSIS")
print("="*60)
...
def extract_activations_gemma(model, tokenizer, data, device='cuda'):
    """Extract activations from Gemma model using hooks."""
    
    model.eval()
    
    # Get model dimensions - Gemma specific
    config = model.config
    num_layers = config.num_hidden_layers  # 28 for Gemma-7B
    num_heads = config.num_attention_heads  # 16 heads for Gemma-7B
    num_kv_heads = config.num_key_value_heads if hasattr(config, 'num_key_value_heads') else num_heads  # 16 KV heads
    hidden_size = config.hidden_size  # 3072 for Gemma-7B
    head_dim = hidden_size // num_heads
    
    print(f"Model config: {num_layers} layers, {num_heads} attention heads, {num_kv_heads} KV heads, {hidden_size} hidden size")
    ...
    def create_layer_hook(layer_idx):
        def hook_fn(module, input, output):
            # For Gemma, we capture the hidden states after the layer
    ...
    def create_attn_hook(layer_idx):
        def hook_fn(module, input, output):
            ...
            if isinstance(output, tuple):
                ...
                attn_output = output
            attn_outputs[layer_idx] = attn_output.detach().cpu()
        return hook_fn
    ...
    for i in range(num_layers):
        ...
        attn_hook = layer.self_attn.register_forward_hook(create_attn_hook(i))
    ...
    for item in tqdm(data, desc="Extracting activations"):
        ...
        # Prepare input with Gemma's chat template
        text = item['prompt']
        
        # Apply Gemma's instruction format
        if "it" in MODEL_NAME.lower():
            # Gemma-it uses special formatting
        ...
        if len(layer_acts) == num_layers:
            ...
            # For Gemma, create head features from layer activations
            # This is a simplified approximation
            head_acts = np.zeros((num_layers, num_kv_heads, head_dim))
            for l in range(num_layers):
                # Reshape hidden states to approximate head outputs
                # Gemma uses multi-query attention, so we use KV heads
                reshaped = layer_acts[l].reshape(num_kv_heads, head_dim)
...
def create_layer_importance_chart(layer_results, save_path='gemma_layer_importance_chart.png'):
    """Create visualization of layer importance for Gemma."""
    ...
    ax1.set_title('Gemma-7B Layer-wise Creativity Detection Performance', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Gemma-7B Relative Layer Importance for Creativity Detection', fontsize=14, fontweight='bold')
...
def main():
    """Main pipeline for Gemma creativity ITI."""
    ...
    if Path(SAVE_DIR).exists() and (Path(SAVE_DIR) / 'train_layer_acts.npy').exists():
        ...
        for split in ['train', 'val', 'test']:
            ...
            try:
                ...
                # Extract activations
                layer_wise, head_wise, labels, problem_ids = extract_activations_gemma(
    ...
    create_layer_importance_chart(layer_results, save_path='gemma_layer_importance_chart.png')
    ...
    print("\n" + "="*60)
    print("GEMMA CREATIVITY ANALYSIS COMPLETE!")
    print("="*60)
    print("\n✅ Chart saved as 'gemma_layer_importance_chart.png'")
    print("📁 Results saved in 'gemma_results/' directory")
